import { Artifact, Case, AIExplanation, ArtifactCategory, PriorityLevel, ConfidenceBreakdown, ValidationResult, ValidationCheck, Fragment, RecoveryStatus, ReconstructionMethod, FragmentType, GroundTruth } from './types';

const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
const USE_MOCK = process.env.NEXT_PUBLIC_USE_MOCK !== 'false';

// Cache for AI Explanations to prevent redundant calls and enable instant live demos
const explanationCache = new Map<string, AIExplanation>();

export function classifyArtifact(filename: string, mimeType: string, previewText: string): ArtifactCategory {
  const text = previewText.toLowerCase();
  if (text.includes("auth_success") || text.includes("sudo") || text.includes("ip address") || text.includes("login")) return 'SYSTEM_TRACE';
  if (filename.endsWith('.csv') || filename.endsWith('.db') || filename.endsWith('.sql')) return 'DATABASE_LOG';
  if (mimeType.startsWith('image/') || filename.endsWith('.png') || filename.endsWith('.jpg')) return 'PHOTO_MEDIA';
  if (filename.endsWith('.exe') || filename.endsWith('.dll') || filename.endsWith('.zip') || mimeType === 'application/octet-stream') return 'BINARY_ARCHIVE';
  return 'DOCUMENT';
}

export function determinePriority(category: ArtifactCategory, previewText: string): PriorityLevel {
  const text = previewText.toLowerCase();
  if (text.includes("password") || text.includes("private key") || text.includes("sudo_exec") || text.includes("admin")) return 'CRITICAL';
  if (category === 'DATABASE_LOG' && (text.includes("amount") || text.includes("transaction") || text.includes("balance"))) return 'HIGH';
  if (category === 'PHOTO_MEDIA') return 'HIGH';
  if (category === 'SYSTEM_TRACE') return 'MEDIUM';
  return 'LOW';
}

/**
 * Adapter function mapping raw FastAPI ArtifactResponse models to the frontend Artifact interface.
 */
export function normalizeArtifact(raw: unknown): Artifact {
  if (!raw || typeof raw !== 'object') {
    throw new Error("Invalid artifact data received from backend");
  }
  const item = raw as Record<string, any>;

  const id = String(item.artifact_id || item.id || 'artifact_unknown');

  const preview_text = typeof item.content_preview === 'string'
    ? item.content_preview
    : typeof item.preview_text === 'string'
    ? item.preview_text
    : '';

  const format = String(item.format || item.mime_type || 'application/octet-stream');
  const metadata = (item.metadata && typeof item.metadata === 'object') ? item.metadata : {};
  const filename = String(metadata.filename || item.filename || `${id}.${format.split('/')[1] || format.split('/')[0] || 'bin'}`);

  const mimeTypeMap: Record<string, string> = {
    txt: 'text/plain',
    csv: 'text/csv',
    png: 'image/png',
  };

  const mime_type = format.includes('/')
    ? format
    : mimeTypeMap[format.toLowerCase()] || `application/${format}`;

  const category: ArtifactCategory = (item.category as ArtifactCategory) || classifyArtifact(filename, mime_type, preview_text);
  const priority: PriorityLevel = (item.priority as PriorityLevel) || determinePriority(category, preview_text);

  const rawBreakdown = item.score_breakdown || item.confidence_breakdown || {};
  const confidence_breakdown: ConfidenceBreakdown = {
    header_validity: Number(rawBreakdown.header_validity) || 0,
    footer_validity: Number(rawBreakdown.footer_validity) || 0,
    structural_validation: Number(rawBreakdown.structural_validation) || 0,
    size_plausibility: Number(rawBreakdown.size_plausibility) || 0,
    reconstruction_integrity: Number(rawBreakdown.reconstruction_integrity) || 0,
    total: Number(rawBreakdown.total ?? item.confidence_score ?? 0),
  };
  const confidence_score = Number(item.confidence_score ?? confidence_breakdown.total);

  const status: RecoveryStatus = (item.status as RecoveryStatus) || 'UNRECOVERABLE';

  const prov = item.provenance || {};
  const verified_bytes = Number(prov.verified_bytes ?? item.verified_bytes ?? 0);
  const reconstructed_bytes = Number(prov.reconstructed_bytes ?? item.reconstructed_bytes ?? 0);
  const missing_bytes = Number(prov.missing_bytes ?? item.missing_bytes ?? 0);

  const rawMethod = prov.reconstruction_method || item.reconstruction_method || 'NONE';
  const reconstruction_method: ReconstructionMethod = (
    rawMethod === 'BIFRAGMENT_GAP' ? 'BIFRAGMENT_GAP' :
    rawMethod === 'CONTIGUOUS' ? 'CONTIGUOUS' :
    (status === 'FULLY_RECOVERED' && reconstructed_bytes === 0) ? 'CONTIGUOUS' : 'NONE'
  );

  const valChecksRaw = Array.isArray(prov.validation_checks)
    ? prov.validation_checks
    : Array.isArray(item.validation?.checks)
    ? item.validation.checks
    : [];

  const checks: ValidationCheck[] = valChecksRaw.map((c: any) => ({
    name: String(c.name || 'Check'),
    detail: String(c.detail || ''),
    passed: Boolean(c.passed),
  }));

  const validation_status = prov.validation_status || item.validation?.valid;
  const valid = validation_status === 'PASSED' || validation_status === true;
  const error = prov.validation_error || item.validation?.error || undefined;

  const validation: ValidationResult = {
    valid,
    ...(error ? { error } : {}),
    ...(checks.length > 0 ? { checks } : {}),
  };

  const rawFragments = Array.isArray(prov.fragments)
    ? prov.fragments
    : Array.isArray(item.fragments)
    ? item.fragments
    : [];

  const fragments: Fragment[] = rawFragments.map((f: any, idx: number) => ({
    id: String(f.id || `frag_${idx + 1}`),
    start_offset: Number(f.start_offset ?? 0),
    end_offset: Number(f.end_offset ?? 0),
    type: (f.type as FragmentType) || 'VERIFIED',
    source_disk: f.source_disk ? String(f.source_disk) : undefined,
  }));

  let ai_summary: AIExplanation | null = null;
  if (item.ai_summary) {
    if (typeof item.ai_summary === 'string') {
      try {
        const parsed = JSON.parse(item.ai_summary);
        if (typeof parsed === 'object' && parsed !== null && parsed.summary) {
          ai_summary = {
            summary: String(parsed.summary),
            details: Array.isArray(parsed.details) ? parsed.details.map(String) : [],
          };
        } else {
          ai_summary = {
            summary: item.ai_summary,
            details: [
              `Category: ${category}`,
              `Status: ${status} (Score: ${confidence_score}/100)`,
              `Reconstruction Method: ${reconstruction_method}`,
            ]
          };
        }
      } catch {
        ai_summary = {
          summary: item.ai_summary,
          details: [
            `Category: ${category}`,
            `Status: ${status} (Score: ${confidence_score}/100)`,
            `Reconstruction Method: ${reconstruction_method}`,
          ]
        };
      }
    } else if (typeof item.ai_summary === 'object' && item.ai_summary.summary) {
      ai_summary = {
        summary: String(item.ai_summary.summary),
        details: Array.isArray(item.ai_summary.details) ? item.ai_summary.details.map(String) : [],
      };
    }
  }

  return {
    id,
    filename,
    mime_type,
    category,
    priority,
    confidence_score,
    confidence_breakdown,
    status,
    verified_bytes,
    reconstructed_bytes,
    missing_bytes,
    reconstruction_method,
    validation,
    fragments,
    preview_text,
    ai_summary,
  };
}

export const mockArtifacts: Artifact[] = [
  {
    // Scenario 4: Bifragment gap ledger.csv
    id: "artifact_001",
    filename: "ledger.csv",
    mime_type: "text/csv",
    category: "DATABASE_LOG",
    priority: "HIGH",
    confidence_score: 78,
    confidence_breakdown: {
      header_validity: 20,
      footer_validity: 0,
      structural_validation: 28,
      size_plausibility: 15,
      reconstruction_integrity: 15,
      total: 78
    },
    status: "PARTIALLY_RECOVERED",
    verified_bytes: 8400,
    reconstructed_bytes: 1200,
    missing_bytes: 400,
    reconstruction_method: "BIFRAGMENT_GAP",
    validation: {
      valid: true,
      checks: [
        { name: "Header Signature", detail: "Valid CSV header found.", passed: true },
        { name: "Row Integrity", detail: "Some rows span gap boundaries, partially recovered.", passed: true }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 8400, type: "VERIFIED" },
      { id: "frag_2", start_offset: 8400, end_offset: 9600, type: "RECONSTRUCTED_GAP" }
    ],
    preview_text: "id,amount,date,status\n1,500.00,2026-09-21,COMPLETED\n2,250.00,2026-09-22,PENDING\n...",
    ai_summary: {
      summary: "Financial ledger with missing trailing records.",
      details: [
        "Recovered: Core transaction history (8400 bytes).",
        "Verified: CSV headers and contiguous block.",
        "Missing: Expected footer/EOF marker (400 bytes missing).",
        "Status reasoning: Received score 78 due to bifragment gap reconstruction.",
        "Priority reasoning: HIGH due to financial nature of the logs."
      ]
    }
  },
  {
    // Scenario 1: Clean/contiguous TXT
    id: "artifact_002",
    filename: "auth_trace.txt",
    mime_type: "text/plain",
    category: "SYSTEM_TRACE",
    priority: "CRITICAL",
    confidence_score: 92,
    confidence_breakdown: {
      header_validity: 20,
      footer_validity: 20,
      structural_validation: 22,
      size_plausibility: 15,
      reconstruction_integrity: 15,
      total: 92
    },
    status: "FULLY_RECOVERED",
    verified_bytes: 1500,
    reconstructed_bytes: 0,
    missing_bytes: 0,
    reconstruction_method: "CONTIGUOUS",
    validation: {
      valid: true,
      checks: [
        { name: "Contiguous Block", detail: "All bytes in single valid block.", passed: true }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 1500, type: "VERIFIED" }
    ],
    preview_text: "2026-09-21T08:15:02Z AUTH_SUCCESS user=admin\n2026-09-21T08:15:05Z SUDO_EXEC cmd=/bin/sh\n...",
    ai_summary: {
      summary: "Critical authentication logs fully recovered.",
      details: [
        "Recovered: Complete authentication trace.",
        "Verified: Entire 1500 byte file is contiguous and intact.",
        "Missing: None.",
        "Status reasoning: 92/100 score as fully recovered, contiguous block.",
        "Priority reasoning: CRITICAL due to evidence of privileged execution."
      ]
    }
  },
  {
    // Scenario 5: Corrupted PNG with bounds check failure
    id: "artifact_003",
    filename: "evidence_capture.png",
    mime_type: "image/png",
    category: "PHOTO_MEDIA",
    priority: "HIGH",
    confidence_score: 34,
    confidence_breakdown: {
      header_validity: 20,
      footer_validity: 0,
      structural_validation: 14,
      size_plausibility: 0,
      reconstruction_integrity: 0,
      total: 34
    },
    status: "CORRUPTED",
    verified_bytes: 2000,
    reconstructed_bytes: 1000,
    missing_bytes: 500,
    reconstruction_method: "NONE",
    validation: {
      valid: false,
      error: "declared chunk exceeds remaining evidence",
      checks: [
        { name: "PNG Header", detail: "Valid PNG magic bytes.", passed: true },
        { name: "IHDR Chunk", detail: "Image header parsed.", passed: true },
        { name: "IDAT Chunk bounds", detail: "Declared chunk exceeds remaining evidence.", passed: false }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 2000, type: "VERIFIED" },
      { id: "frag_2", start_offset: 2000, end_offset: 2500, type: "MISSING" }
    ],
    preview_text: "<PNG binary data unrenderable>",
    ai_summary: {
      summary: "Corrupted image file with truncated payload.",
      details: [
        "Recovered: Partial image headers.",
        "Verified: Valid PNG magic bytes.",
        "Missing: Image payload and footer.",
        "Status reasoning: CORRUPTED due to invalid IDAT bounds check.",
        "Priority reasoning: HIGH as potential photo evidence."
      ]
    }
  },
  {
    // Scenario 6: Unrecoverable fragment
    id: "artifact_004",
    filename: "damaged_sector.txt",
    mime_type: "text/plain",
    category: "DOCUMENT",
    priority: "LOW",
    confidence_score: 12,
    confidence_breakdown: {
      header_validity: 0,
      footer_validity: 0,
      structural_validation: 12,
      size_plausibility: 0,
      reconstruction_integrity: 0,
      total: 12
    },
    status: "UNRECOVERABLE",
    verified_bytes: 100,
    reconstructed_bytes: 0,
    missing_bytes: 900,
    reconstruction_method: "NONE",
    validation: {
      valid: false,
      error: "no valid header found",
      checks: [
        { name: "Magic Bytes", detail: "No valid header found.", passed: false }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 100, type: "VERIFIED" }
    ],
    preview_text: "\\x00\\x00\\x00... (unintelligible sector noise)",
    ai_summary: {
      summary: "Completely unrecoverable damaged sector.",
      details: [
        "Recovered: None.",
        "Verified: 100 bytes of raw data, completely unrecognizable.",
        "Missing: Essentially all content (900 expected bytes).",
        "Status reasoning: UNRECOVERABLE due to lack of headers and structure.",
        "Priority reasoning: LOW because no intelligible data exists."
      ]
    }
  },
  {
    // Scenario 2: Deleted TXT with boundary note
    id: "artifact_005",
    filename: "deleted_notes.txt",
    mime_type: "text/plain",
    category: "DOCUMENT",
    priority: "MEDIUM",
    confidence_score: 86,
    confidence_breakdown: {
      header_validity: 20,
      footer_validity: 20,
      structural_validation: 16,
      size_plausibility: 15,
      reconstruction_integrity: 15,
      total: 86
    },
    status: "FULLY_RECOVERED",
    verified_bytes: 350,
    reconstructed_bytes: 0,
    missing_bytes: 0,
    reconstruction_method: "CONTIGUOUS",
    validation: {
      valid: true,
      checks: [
        { name: "File Structure", detail: "Recovered via contiguous carving.", passed: true }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 350, type: "VERIFIED" }
    ],
    preview_text: "[SYNTHETIC_ARTIFACT_START]\nMeet at 9PM to discuss the transfer.\n[SYNTHETIC_ARTIFACT_END]",
    ai_summary: {
      summary: "Deleted text document recovered successfully.",
      details: [
        "Recovered: Complete text content.",
        "Verified: Carved using synthetic start and end markers.",
        "Missing: None.",
        "Status reasoning: 86/100 score, fully recovered.",
        "Priority reasoning: MEDIUM due to suspicious meeting note."
      ]
    }
  },
  {
    // Scenario 3: Fragmented CSV
    id: "artifact_006",
    filename: "contacts.csv",
    mime_type: "text/csv",
    category: "DATABASE_LOG",
    priority: "MEDIUM",
    confidence_score: 65,
    confidence_breakdown: {
      header_validity: 20,
      footer_validity: 0,
      structural_validation: 20,
      size_plausibility: 10,
      reconstruction_integrity: 15,
      total: 65
    },
    status: "PARTIALLY_RECOVERED",
    verified_bytes: 2048,
    reconstructed_bytes: 1024,
    missing_bytes: 1024,
    reconstruction_method: "BIFRAGMENT_GAP",
    validation: {
      valid: true,
      checks: [
        { name: "Header Signature", detail: "Valid CSV header found.", passed: true },
        { name: "Row Integrity", detail: "Some rows corrupted across fragmented gap.", passed: false }
      ]
    },
    fragments: [
      { id: "frag_1", start_offset: 0, end_offset: 1024, type: "VERIFIED" },
      { id: "frag_2", start_offset: 1024, end_offset: 2048, type: "RECONSTRUCTED_GAP" },
      { id: "frag_3", start_offset: 2048, end_offset: 3072, type: "VERIFIED" }
    ],
    preview_text: "name,email,phone\nJohn Doe,john@example.com,555-0101\n...<GAP>...Jane Doe,jane@example.com,555-0102\n",
    ai_summary: {
      summary: "Fragmented contacts list.",
      details: [
        "Recovered: Most contact rows.",
        "Verified: Header and trailing block.",
        "Missing: Middle gap containing ~1024 bytes.",
        "Status reasoning: 65/100 score due to significant unrecovered gap.",
        "Priority reasoning: MEDIUM for PII discovery."
      ]
    }
  }
];

export const mockCase: Case = {
  id: "case_001",
  name: "Operation Phantom",
  description: "Investigation into unauthorized access.",
  artifacts: mockArtifacts
};

export const mockGroundTruth: GroundTruth = {
  case_id: "case_001",
  image_filename: "phantom_disk.img",
  expected_artifacts: [
    {
      id: "artifact_002",
      filename: "auth_trace.txt",
      scenario: 'CLEAN_CONTIGUOUS',
      expected_status: 'FULLY_RECOVERED',
      original_sha256: "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855",
      total_bytes: 1500
    },
    {
      id: "artifact_005",
      filename: "deleted_notes.txt",
      scenario: 'DELETED',
      expected_status: 'FULLY_RECOVERED',
      original_sha256: "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce",
      total_bytes: 350
    },
    {
      id: "artifact_006",
      filename: "contacts.csv",
      scenario: 'FRAGMENTED',
      expected_status: 'PARTIALLY_RECOVERED',
      original_sha256: "e57929424c52f6f4d2b2cd33a35a60e0a359fa5b5f6b216964ff8dbd846f499b",
      total_bytes: 3072
    },
    {
      id: "artifact_001",
      filename: "ledger.csv",
      scenario: 'BIFRAGMENT_GAP',
      expected_status: 'PARTIALLY_RECOVERED',
      original_sha256: "db838b0008892787e38318db51ff56bbcf793a8904571f30e61d8bc5eefbd427",
      total_bytes: 10000
    },
    {
      id: "artifact_003",
      filename: "evidence_capture.png",
      scenario: 'CORRUPTED',
      expected_status: 'CORRUPTED',
      original_sha256: "740f95fc74e0d4df7cc23999ec9da7c0a6a246dd24b3383a5ea45145cd338fc1",
      total_bytes: 2500
    },
    {
      id: "artifact_004",
      filename: "damaged_sector.txt",
      scenario: 'UNRECOVERABLE',
      expected_status: 'UNRECOVERABLE',
      original_sha256: "deadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef",
      total_bytes: 1000
    }
  ]
};

async function handleResponseError(res: Response): Promise<never> {
  let detail = res.statusText;
  try {
    const text = await res.text();
    try {
      const errJson = JSON.parse(text);
      if (errJson.detail) {
        detail = typeof errJson.detail === 'string' ? errJson.detail : JSON.stringify(errJson.detail);
      } else if (text) {
        detail = text;
      }
    } catch {
      if (text) detail = text;
    }
  } catch {
    // res.text failed
  }
  throw new Error(`API Error ${res.status}: ${detail}`);
}

export async function createCase(name?: string, description?: string): Promise<Case> {
  if (USE_MOCK) {
    return { ...mockCase, name: name || mockCase.name };
  }
  const res = await fetch(`${API_BASE}/api/cases`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ name: name || "New Case", description }),
    cache: 'no-store'
  });
  if (!res.ok) {
    await handleResponseError(res);
  }
  const data = await res.json();
  return {
    id: data.case_id || data.id,
    name: data.name || name || "New Case",
    description: data.description || description || "",
    artifacts: []
  };
}

export async function uploadEvidence(caseId: string, fileOrSample: File | string): Promise<{ success: boolean }> {
  if (USE_MOCK) {
    return { success: true };
  }
  const formData = new FormData();
  if (typeof fileOrSample === 'string') {
    const blob = new Blob([fileOrSample], { type: 'text/plain' });
    formData.append('file', blob, 'sample_evidence.txt');
  } else {
    formData.append('file', fileOrSample);
  }
  const res = await fetch(`${API_BASE}/api/cases/${caseId}/evidence`, {
    method: 'POST',
    body: formData,
    cache: 'no-store'
  });
  if (!res.ok) {
    await handleResponseError(res);
  }
  return { success: true };
}

export async function analyzeCase(caseId: string): Promise<Case> {
  if (USE_MOCK) {
    return mockCase;
  }
  const res = await fetch(`${API_BASE}/api/cases/${caseId}/analyze`, {
    method: 'POST',
    cache: 'no-store'
  });
  if (!res.ok) {
    await handleResponseError(res);
  }
  const data = await res.json();
  const rawArtifacts = Array.isArray(data.artifacts) ? data.artifacts : [];
  const normalizedArtifacts = rawArtifacts.map(normalizeArtifact);

  let caseName = "Case " + caseId;
  let caseDesc = "";
  try {
    const cData = await fetchCase(caseId);
    caseName = cData.name;
    caseDesc = cData.description;
  } catch {
    // fallback
  }

  return {
    id: data.case_id || caseId,
    name: caseName,
    description: caseDesc,
    artifacts: normalizedArtifacts
  };
}

export async function runFullIngestionAndAnalysis(fileOrSample: File | string, caseName = "Automated Case"): Promise<Case> {
  const newCase = await createCase(caseName);
  await uploadEvidence(newCase.id, fileOrSample);
  return await analyzeCase(newCase.id);
}

export async function fetchCase(caseId = 'case_001'): Promise<Case> {
  if (USE_MOCK) {
    return mockCase;
  }
  try {
    const res = await fetch(`${API_BASE}/api/cases/${caseId}`, { cache: 'no-store' });
    if (!res.ok) {
      await handleResponseError(res);
    }
    const caseData = await res.json();
    let artifacts: Artifact[] = [];
    try {
      artifacts = await fetchArtifacts(caseId);
    } catch {
      // If fetching artifacts specifically fails, fallback to empty array
    }
    return {
      id: caseData.case_id || caseData.id || caseId,
      name: caseData.name || "Forensic Recovery Case",
      description: caseData.description || "",
      artifacts
    };
  } catch {
    return {
      ...mockCase,
      id: caseId,
      artifacts: await fetchArtifacts(caseId).catch(() => mockArtifacts)
    };
  }
}

export async function fetchArtifacts(caseId = 'case_001'): Promise<Artifact[]> {
  if (USE_MOCK) {
    return mockArtifacts;
  }
  const res = await fetch(`${API_BASE}/api/cases/${caseId}/artifacts`, { cache: 'no-store' });
  if (!res.ok) {
    await handleResponseError(res);
  }
  const rawArtifacts = await res.json();
  if (!Array.isArray(rawArtifacts)) return [];
  return rawArtifacts.map(normalizeArtifact);
}

export async function fetchArtifactById(artifactId: string): Promise<Artifact> {
  if (USE_MOCK) {
    const artifact = mockArtifacts.find(a => a.id === artifactId);
    if (!artifact) throw new Error("Artifact not found");
    return artifact;
  }
  const res = await fetch(`${API_BASE}/api/artifacts/${artifactId}`, { cache: 'no-store' });
  if (!res.ok) {
    await handleResponseError(res);
  }
  const raw = await res.json();
  return normalizeArtifact(raw);
}

export async function fetchArtifactExplanation(artifactId: string): Promise<AIExplanation> {
  if (explanationCache.has(artifactId)) {
    return explanationCache.get(artifactId)!;
  }

  if (USE_MOCK) {
    let explanation: AIExplanation | null = null;
    const artifact = mockArtifacts.find(a => a.id === artifactId);
    if (artifact && artifact.ai_summary) {
      explanation = artifact.ai_summary;
    }
    if (!explanation) {
      explanation = {
        summary: "Artifact evidence breakdown.",
        details: ["No additional AI explanation available for this artifact."]
      };
    }
    explanationCache.set(artifactId, explanation);
    return explanation;
  }

  try {
    const res = await fetch(`${API_BASE}/api/artifacts/${artifactId}/explain`, {
      method: 'POST',
      cache: 'no-store'
    });
    if (!res.ok) {
      await handleResponseError(res);
    }
    const data = await res.json();
    const explanation: AIExplanation = {
      summary: String(data.summary || "Artifact evidence breakdown."),
      details: Array.isArray(data.details) ? data.details.map(String) : []
    };
    explanationCache.set(artifactId, explanation);
    return explanation;
  } catch (e) {
    const artifact = await fetchArtifactById(artifactId).catch(() => null);
    if (artifact && artifact.ai_summary) {
      explanationCache.set(artifactId, artifact.ai_summary);
      return artifact.ai_summary;
    }
    const fallback: AIExplanation = {
      summary: "Artifact evidence breakdown.",
      details: ["No additional AI explanation available for this artifact."]
    };
    return fallback;
  }
}

export async function fetchGroundTruth(caseId = 'case_001'): Promise<GroundTruth> {
  if (USE_MOCK) {
    return mockGroundTruth;
  }
  try {
    const res = await fetch(`${API_BASE}/api/cases/${caseId}/groundtruth`, { cache: 'no-store' });
    if (res.ok) return await res.json();
  } catch {
    // Fallback if groundtruth route is not available on backend
  }
  return mockGroundTruth;
}
